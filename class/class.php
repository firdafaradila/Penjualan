<?php  
	...
	class Admin{
		...
		public function simpan_admin($email,$pass,$nama,$gambar){
			...
			//insert
			mysql_query("INSERT INTO admin(email,password,nama,gambar,level) VALUES('$email','$pass','$nama','$namafile','admin')");
		}
		public function tampil_admin(){
			$qry = mysql_query("SELECT * FROM admin WHERE level = 'admin'");
		...
		public function tampil_admin_byid($id){
	...
	class Supplier{
		...
		public function tampil_supplier_byid($id){
			$qry = mysql_query("SELECT * FROM supplier WHERE kd_admin = '$id'");
			while ($pecah = mysql_fetch_array($qry)) {
				$data[] = $pecah;
			}
			return $data;
		}
		public function simpan_supplier($nama,$alamat,$email,$password,$gambar){
			$namafile = $gambar['name'];
			//lokasi sementara
			$lokasifile = $gambar['tmp_name'];
			//upload
			...
			mysql_query("INSERT INTO admin(nama,email,password,gambar,level) VALUES('$nama','$email','$password','$namafile','$supplier')");

			$qry = mysql_query("SELECT * FROM admin WHERE email = '$email'");

			$user = mysql_fetch_assoc($qry);

			mysql_query("INSERT INTO supplier(kd_admin,nama_supplier,alamat) VALUES('$user[kd_admin]','$nama','$alamat')");
		}
	...
	class Pembelian{
		...
		public function tampil_pembelian($id){
			if ($id == null) {
	...
	class Nota{
		...
		public function upload_pembayaran($kd, $gambar){
			$namafile = $gambar['name'];
			//lokasi sementara
			$lokasifile = $gambar['tmp_name'];
			//upload
			move_uploaded_file($lokasifile, "gambar_bayar/$namafile");
			
			$qry = mysql_query("UPDATE pembelian SET bukti_pembayaran = '111',
				SET status_bayar = '1'
				WHERE kd_pembelian = '$kd'");
		}
